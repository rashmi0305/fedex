...
# Optimized Greedy Algorithm to allocate packages to ULDs
def fit_packages_to_uld(uld_df, package_df):
    ...
    # Sort packages by type (priority), weight, and volume for better packing
    package_df["Volume"] = package_df["Length"] * package_df["Width"] * package_df["Height"]
    package_df = package_df.sort_values(by=["Type", "Weight"], ascending=[False, False])
    allocations_result = []
